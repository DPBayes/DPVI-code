...
import pickle
data = pickle.load(open('/home/jalko/Dropbox/code/icml/inferences/logistic/adult/data/adult.p','rb'))
data = pickle.load(open('../logistic/adult/data/adult.p','rb'))
...
[accs_advi, iter_sigmas] = pickle.load(open('./data/adult_advi_acc_sigmas.p', 'rb'))
[np_acc] = pickle.load(open('../logistic/adult/data/adult_ndp.p','rb'))
...
delta = 1e-3
delta_prime = 4*1e-5
eps_tot = 2*act(2*iter_sigmas, 0.5*delta, delta_prime, T_advi, q_advi)
n_ave = 10
i = 0
...
pickle.dump([accs_sgld, eps_tot, 0.001], open('./data/adult_sgld_acc.p', 'wb'))
[accs_sgld, eps_tot, delta] =  pickle.load(open('./data/adult_sgld_acc.p', 'rb'))
...
for sigma in iter_sigmas:
	eps_tot_ma.append(ma(sigma, 0.5*delta, T_advi, q_advi, 160))
...
pp = PdfPages('./plots/sgld_vs_advi_adult.pdf')
...
plt.xscale('log')
pp.savefig(plot1)
plt.savefig("./plots/sgld_vs_advi_adult.png")
pp.close()
plt.close()
