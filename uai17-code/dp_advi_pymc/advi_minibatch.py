...
def _make_logpt(global_RVs, local_RVs, observed_RVs, model):
    ...
    """
    factors = [s*v.logp_elemwiset for v, s in observed_RVs.items()] + [v.logp_elemwiset for v in global_RVs] + model.potentials
    w_lfactors = factors[0].reshape([factors[0].size])
    w_gfactors = tt.stack(factors[1:])
    #if local_RVs is not None:
        #factors += [s * v.logp_elemwiset for v, (_, s) in local_RVs.items()]
        #w_lfactors = tt.stack(factors[:8])
    #logpt = tt.add(*map(tt.sum, factors))
    logpt =  tt.concatenate([w_gfactors, w_lfactors]) 
...
def _elbo_t(logp, uw_g, uw_l, inarray_g, inarray_l, n_mcsamples, random_seed):
    ...
    if uw_l is not None:
        sys.exit("Current implementation works only if local variables is none: Exit")
        ...
        if n_mcsamples == 1:
            ...
            #z_g = tt.exp(w_g) + u_g
            z_g = n_g * tt.exp(w_g) + u_g
            elbo = [logp_(z_g), tt.sum(w_g) + 0.5 * l_g * (1 + np.log(2.0 * np.pi))]
        else:
            ...
            logps, _ = theano.scan(fn=lambda q: logp_(q),
                                   outputs_info=None,
                                   sequences=[zs_g])
            elbo = [tt.mean(logps, axis=0), tt.sum(w_g) + 0.5 * l_g * (1 + np.log(2.0 * np.pi))]
...
def advi_minibatch(vars=None, start=None, model=None, n=5000, n_mcsamples=1,
                   ...
                   epsilon=.1, random_seed=None, verbose=1, dp_par=None):
    ...
    for param in params:
        ...
        updates.update(optimizer(likeloss=-1 * elbo[0], entroloss = -1*elbo[1],param=param, dp_par=dp_par, n_par=len(vars)))
    f = theano.function(tensors, tt.add(elbo[1],tt.sum(elbo[0], axis=0)), updates=updates)
